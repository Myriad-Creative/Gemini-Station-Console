...
function normalizeMission(raw: RawMission, idFallback: string, mobIndex: Map<string, Mob>): Mission | null {
  if (!raw) return null;
...
export function parseMissions(repoRoot: string, mobIndex: Map<string, Mob>): Mission[] {
  ...
  for (const f of files) {
    ...
    const mission = normalizeMission(raw, path.basename(f, ".json"), mobIndex);
    if (mission) missions.push(mission);
  }
  return missions;
}

export function parseMissionsFromData(raw: RawMission[] | Record<string, RawMission> | null, mobIndex: Map<string, Mob>): Mission[] {
  if (!raw) return [];
  const arr: RawMission[] = Array.isArray(raw) ? raw : Object.values(raw);
  const missions: Mission[] = [];
  for (const m of arr) {
    const mission = normalizeMission(m, String(m?.id ?? missions.length + 1), mobIndex);
    if (mission) missions.push(mission);
  }
  return missions;
}
